((exports) => {
  ...
  const iterateList = ({ values }, handler) =>
    values.forEach((value, index) => handler(value, index));
  const getListSize = ({ values }) => values.length;
  ...
  const getStorageSize = ({ keys }) => keys.length;
